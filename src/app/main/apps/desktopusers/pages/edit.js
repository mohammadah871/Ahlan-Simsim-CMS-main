...
import Form from './../../forms/desktopuser'
...
function Edit(props) {
    ...
    const submitData = (dataVal) => {
        let query = `
        mutation {
            UpdateDesktopUserSingle(userAppDesktopInput:{
                id:${id},
                email:"${dataVal.email}",
                fname:"${dataVal.fname}",
                lname:"${dataVal.lname}",
                gender:"${dataVal.gender}",
                accountType:"${dataVal.accountType}",
        ...
        axios
            .post(
                ...
                navigate(baseUrl + "desktop_accounts")
            });
    }
    ...
    useEffect(() => {
        ...
        axios
            .post(
                `${apiBaseUrl}/graphql`,
                {
                    "query": "query request($data: UserAppDesktopListInput) {  GetDesktopUserSingle(userAppDesktopListInput: $data) { id,email,fname,lname,gender,accountType,phoneNo }}",
                    "variables": {
                        "data": {
                            "id": parseInt(id)
                        }
                ...
                setData(res.data.data.GetDesktopUserSingle);
            });
    }, [])
    return (
        <FusePageCarded
